...
def _open_tar_from_bytes(b: bytes):
    """Open a tar.gz archive from a bytes buffer; return the TarFile."""
    import io
    import tarfile

    return tarfile.open(fileobj=io.BytesIO(b), mode="r:gz")


def test_admin_backup_default_full(mcp_client: TestClient):
    """Default contents=full returns a tar.gz with every file under
    SMALT_DIR — including the LanceDB index and pages. (Note: this
    test runs before any test_bootstrap_* in the session, so the
    `schema/SCHEMA.md` placeholder hasn't been created yet — we only
    assert what conftest's seed + ensure_tables guarantees.)"""
    r = mcp_client.get("/admin/backup")
    assert r.status_code == 200
    assert r.headers["content-type"] == "application/gzip"
    # Filename follows `smalt-<dir>-<utc-iso>.tar.gz` shape.
    cd = r.headers["content-disposition"]
    assert cd.startswith("attachment; ")
    assert "smalt-" in cd
    assert cd.endswith('.tar.gz"')
    # Bytes form a well-formed gzip tar; unpacks; contains pages + index.
    tar = _open_tar_from_bytes(r.content)
    names = set(tar.getnames())
    tar.close()
    # Seed pages are present.
    assert any(n == "pages/entities/alice.md" for n in names), names
    assert any(n == "pages/concepts/embedding.md" for n in names)
    # LanceDB index dir is present in full mode (some path under index/lance/).
    assert any(n.startswith("index/lance/") for n in names)


def test_admin_backup_contents_no_index(mcp_client: TestClient):
    """contents=no-index excludes the LanceDB store; keeps pages."""
    r = mcp_client.get("/admin/backup?contents=no-index")
    assert r.status_code == 200
    tar = _open_tar_from_bytes(r.content)
    names = set(tar.getnames())
    tar.close()
    assert any(n == "pages/entities/alice.md" for n in names)
    # NO files under index/lance/.
    assert not any(n.startswith("index/lance/") for n in names), (
        f"index/lance/ files leaked into no-index backup: "
        f"{[n for n in names if n.startswith('index/lance/')]}"
    )


def test_admin_backup_contents_pages_only(mcp_client: TestClient):
    """contents=pages-only restricts to pages/, structures/, schema/."""
    r = mcp_client.get("/admin/backup?contents=pages-only")
    assert r.status_code == 200
    tar = _open_tar_from_bytes(r.content)
    names = list(tar.getnames())
    tar.close()
    # Every entry must be under pages/, structures/, or schema/.
    allowed_prefixes = ("pages/", "structures/", "schema/")
    for n in names:
        assert n.startswith(allowed_prefixes), (
            f"pages-only backup contained off-scope file {n!r}"
        )
    # Pages are present.
    assert any(n == "pages/entities/alice.md" for n in names)


def test_admin_backup_filename_includes_timestamp(mcp_client: TestClient):
    """The Content-Disposition filename includes a UTC-ISO timestamp
    (compact form: YYYYMMDDTHHMMSSZ)."""
    import re

    r = mcp_client.get("/admin/backup")
    cd = r.headers["content-disposition"]
    # Match the 16-char compact-ISO timestamp.
    assert re.search(r"\d{8}T\d{6}Z", cd), f"no UTC-ISO timestamp in {cd!r}"


def test_admin_backup_rejects_invalid_format(mcp_client: TestClient):
    r = mcp_client.get("/admin/backup?format=zip")
    assert r.status_code == 400
    assert "tar.gz" in r.json()["detail"]


def test_admin_backup_rejects_invalid_contents(mcp_client: TestClient):
    r = mcp_client.get("/admin/backup?contents=everything")
    assert r.status_code == 400
    assert "contents" in r.json()["detail"].lower()


def test_admin_backup_archive_round_trips_a_page(mcp_client: TestClient, tmp_path):
    """Extract the tar.gz to a tmp dir; read back ent-alice.md; verify it
    has the seed content."""
    import io
    import tarfile
    ...
    alice = (out_dir / "pages" / "entities" / "alice.md").read_text(encoding="utf-8")
    # The seed has "Alice is a fictional person used in the seed Smalt".
    assert "Alice is a fictional person" in alice
    # Frontmatter shows the seed id.
    assert "id: ent-alice" in alice


def test_admin_backup_not_double_gzipped(mcp_client: TestClient):
    """GZipMiddleware mustn't re-compress the already-gzipped tar.
    Confirm Content-Encoding: identity is set, and the bytes are
    well-formed gzip (a double-gzip would have two gzip headers, and the
    outer one would decompress to gzip — not tar)."""
    r = mcp_client.get("/admin/backup", headers={"Accept-Encoding": "gzip"})
    assert r.status_code == 200
    assert r.headers.get("content-encoding") == "identity"
    # First bytes are the gzip magic number (1f 8b).
    assert r.content[:2] == b"\x1f\x8b"
    # And it's a tar archive once decompressed (not another gzip header).
    import io
    import tarfile

    with tarfile.open(fileobj=io.BytesIO(r.content), mode="r:gz") as tar:
        # Just listing succeeds — proves it's tar, not gzip-of-gzip.
        names = tar.getnames()
    assert len(names) > 0
